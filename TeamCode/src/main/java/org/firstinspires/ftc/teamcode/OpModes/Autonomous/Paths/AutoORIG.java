...
@Autonomous(name = "AutoORIG")
public class AutoORIG extends OpMode {
    ...
    private void updatePathState() {
        switch (stage) {
            case 0:
                ...
                } else if (!stageActionTriggered && follower.getPathCompletion() > 0.4) {
                    applyOptionsAction();
                    stageActionTriggered = true;
                }
            ...
            case 3:
                if (!pathStarted) {
                    applyRightBumperAction();
                    startPath(paths.Path4, SCORE_APPROACH_MAX_POWER);
                }
                if (pathStarted && !follower.isBusy()) {
                    waitForScoreAndAdvance(4);
                }
                break;
            case 4:
                if (!pathStarted) {
                    startPath(paths.Path5, 1.0);
                } else if (!stageActionTriggered && follower.getPathCompletion() > 0.15) {
                    applyTravelResetActions();
                    stageActionTriggered = true;
                }
                if (pathStarted && !follower.isBusy()) {
                    advanceToStage(5);
            ...
            case 7:
                if (!pathStarted) {
                    applyRightBumperAction();
                    startPath(paths.Path8, SCORE_APPROACH_MAX_POWER);
                }
                if (pathStarted && !follower.isBusy()) {
                    waitForScoreAndAdvance(8);
                }
                break;
            case 8:
                if (!pathStarted) {
                    startPath(paths.Path9, 1.0);
                } else if (!stageActionTriggered && follower.getPathCompletion() > 0.15) {
                    applyTravelResetActions();
                    stageActionTriggered = true;
                }
            ...
            case 11:
                if (!pathStarted) {
                    applyRightBumperAction();
                    startPath(paths.Path12, SCORE_APPROACH_MAX_POWER);
                }
                if (pathStarted && !follower.isBusy()) {
                    waitForScoreAndAdvance(12);
                }
                break;
            case 12:
                if (!pathStarted) {
                    startPath(paths.Path13, 1.0);
                } else if (!stageActionTriggered && follower.getPathCompletion() > 0.15) {
                    applyTravelResetActions();
                    stageActionTriggered = true;
                }
